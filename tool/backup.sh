...
# =============================================================================
# 函数名称: _fail
# 功能描述: 备份失败的终止型错误输出 (走 stderr, 不污染数据输出流)。
# 参数: $1=错误消息; $2=可选的可执行建议 (非空时以 [建议] 追加一行到 stderr)
# 返回值: 不返回 (exit 1)
# 注: _info / _warn 已下沉到 core/_common.sh (print_info / print_warn 的短名别名),
#     此处不再内联副本 (原实现用 echo -e, 与共享版 printf 输出等价且后者更可预测)。
#     本文件的 _fail 与 core/check.sh 的同名函数**语义不同** (check 的仅打印、不退出),
#     故保留独立定义, 未并入共享库。
# =============================================================================
function _fail() {
