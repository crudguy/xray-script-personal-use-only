...
# exec_menu 桩件: 仅 `--index` 从队列文件头部弹出一个值; banner/status 返回空。
#   队列状态持久化到文件, 以跨越命令替换 subshell 的变量隔离。
exec_menu() {
    case "$1" in
        --index)
            echo "index_call" >> "$CALLS"
            local first rest
...
printf '9\n0\n' > "$QUEUE"; : > "$CALLS"
( processes_index ); rcA=$?
idxA=$(grep -c index_call "$CALLS" 2>/dev/null || true)
cfgA=$(grep -c config_called "$CALLS" 2>/dev/null || true)
[[ $rcA -eq 0 ]] || { echo "[FAIL A] 退出码应为 0, 实际 $rcA"; ok=0; }
# 2 次 index_call = 迭代1(选9) + 迭代2(选0), 证明选 9 后回到主菜单重读了一次选择
[[ $idxA -eq 2 ]] || { echo "[FAIL A] exec_menu --index 应被调用 2 次 (主循环回到主菜单), 实际 $idxA"; ok=0; }
