...
#   T4 守卫: 日志函数 (下沉后的唯一副本 core/_common.sh) 已参数化
#            —— 锚点随下沉从 service/*.sh 改为 _common.sh, 否则"副本已删除"会被误判成"修法丢失"
#   T5 守卫: 已下沉的 print_* / cmd_exists 等不得再出现重复定义 (install.sh 自包含豁免)
#   T6 守卫: cmd_exists 不得再走 eval (实测 `eval type "$cmd"` 会执行注入的命令)
#
