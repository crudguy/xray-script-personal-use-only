...
#
# 为什么需要它:
#   本目录已有 81 个用例, 而 ok/bad 被重复定义了 37 次、assert_eq 被重复定义了 36 次 ——
#   每个文件各抄一份, 于是"失败信息里期望/实际标反"这类问题只能逐个文件发现
#   (历史上一处标反排查方向被带偏过一次)。新增用例统一从这里 source, 不再各自抄。
#
# 存量用例**刻意不迁移** (2026-09-26 定, 详见 test/README.md):
#   81 个用例里只有 3 个 source 本文件 (handler_arms_coverage / heal_mkcp /
#   mkcp_finalmask), 其余 78 个仍各写一份 ok/bad/assert_eq —— 这是权衡后的选择,
#   不是漏做。理由:
#     - 存量用例各自带着为自己定制的桩件与负向校验 (不少是针对某个具体坑写的),
#       整批换过来的收益只是少几行重复定义, 风险却是把已稳定的断言语义动一遍;
#     - 负向校验最怕的失效是"断言被改写成恒绿" —— 它仍会绿着, 而全量测试只能证明
#       "没变红", 证明不了"没变恒绿"。
#   只有三种情况值得迁一个存量用例: ① 它正在被大改 (顺手迁, 成本摊在大改里);
#   ② 它要新接 config.env 里已有的夹具 (迁了才真复用得上); ③ 重复定义确实造成了
#   问题 (如某处期望/实际又标反了) —— 定点修, 不动其它。
#   本文件只保证**新增**用例有一致的行为与输出格式。
#
# 约定 (与全仓一致, 不要改):
