...
# 注 1: 必须用 ANSI-C 引号 $'...' 让 \033 成为真正的 ESC 控制字符, 而非字面反斜杠序列。
#       否则在 printf '%s' "${GREEN}" 这类"转义在参数里"的调用中会被原样打印成
#       "\033[32m" 乱码 (菜单的 echo -e 路径能解释, 但体检查看的 printf %s 不能)。
# 注 2: 层级约定 —— 菜单与体检输出共用这一套, 加新色前先看这里:
#         GREEN  选项/条目本身 (可操作的那一行)
#         CYAN   说明文字 (解释上面选项的次级信息)
#         YELLOW 提示 / 警告 (需留意, 但不阻断)
#         RED    退出项 / 失败
#       说明文字单独占一个颜色的原因: 它与选项同属菜单正文、同样以 "N. " 起头,
#       仅靠一条分隔线区分时, 视线扫过很容易把"解释"误读成"又一项可选项"。
#       选 CYAN 而非"置暗 (\033[2m)": 置暗靠降低与背景的对比度来退后, 在深色/浅色
#       两类终端里总有一边读不清; CYAN 在两种配色下都能与默认前景色拉开差距且保持可读。
readonly GREEN=$'\033[32m'  # 绿色
readonly CYAN=$'\033[36m'   # 青色 (说明文字)
...
#   core/check.sh  _check_info/_check_pass/_check_fail -> 体检专用报告语汇: 信息用黄
#                  (与"通过"绿在视觉上分开), 失败仅打印不退出 (要跑完全部检查项再汇总);
#                  加 _check_ 前缀即与本文件的短名彻底分开, 不再有同名遮蔽
