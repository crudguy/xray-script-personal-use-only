...
# =============================================================================
# 函数名称: exec_menu
# 功能描述: 执行菜单脚本 (menu.sh)，把用户的选择编号打印到 stdout 供调用方读取。
# 参数:
#   $@: 传递给 menu.sh 脚本的参数
# 返回值: 恒 0 (选择编号走 stdout, 由调用方 `choose="$(exec_menu ...)"` 取用)
# =============================================================================
...
#           2. 显示 SNI 配置菜单。
#           3. 根据用户选择执行不同的 SNI 相关操作: 1 改域名, 2 改 CDN, 3 强制续签,
#              4 更新 Nginx 配置, 5 Nginx Cron, 6 Web 配置, 7 切换 CA 厂商,
#              8 自定义站点, 9 移除单域名证书。
#              (早期注释里的"重置 V3"在当前菜单中并无对应项, 已删除。)
# 参数: 无
# 返回值: 无 (通过调用其他函数和脚本执行操作)
...
# 功能描述: 处理主配置管理相关的流程。
#           1. 显示主配置管理菜单。
#           2. 根据用户选择进入不同的子流程或执行操作: 1 Xray 配置, 2 路由规则,
#              3 SNI 配置, 4 改端口, 5 GeoData Cron, 6 语言, 7 BBR 与内核网络,
#              8 配置备份与迁移, 9 嗅探仅用于路由。
# 参数: 无
# 返回值: 无 (通过调用其他函数和脚本执行操作)
# =============================================================================
...
#              4 进程文件句柄上限; 5 只读检测 IPv6 状态; 6 进入 IPv6 启停子菜单。
#              3~6 都不是"BBR"本身: 前者影响面覆盖整机, 后两项是独立的网络协议
#              族开关, 故一律各自成项, 不并入"开启 BBR", 也不自动执行。
# 参数: 无
...
# 功能描述: 处理脚本主界面的流程。
#           1. 显示 Banner、状态和主菜单。
#           2. 根据用户选择执行不同的主操作 (一键安装, Xray 安装, 卸载, 启动, 停止, 重启,
#              分享链接, 流量统计, 配置管理, 全量体检, 生成订阅)。
# 参数: 无
# 返回值: 无 (通过调用其他函数和脚本执行操作)
# =============================================================================
